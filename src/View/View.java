...
public class View {
    ...
    private void runProgram(InterfaceStatement stmt, String logFilePath) throws ADTException, ExprEvalException, StmtExecException, IOException {
        InterfaceStack<InterfaceStatement> exeStack = new Stack<>();
        InterfaceDictionary<String, InterfaceValue> symTable = new Dictionary<>();
        InterfaceList<InterfaceValue> out = new List<>();
        InterfaceDictionary<String, BufferedReader> fileTable = new Dictionary<>();
        ProgramState prgState = new ProgramState(exeStack, symTable, out, stmt, fileTable);
