...
public class Heap implements InterfaceHeap {
    ...
    @Override
    public void setContent(HashMap<Integer, InterfaceValue> newMap) {
        this.heap = newMap;
    }

    @Override
    public int add(InterfaceValue value) {
    ...
    @Override
    public void update(Integer position, InterfaceValue value) throws ADTException {
    ...
    @Override
    public InterfaceValue get(Integer position) throws ADTException {
        if (!heap.containsKey(position))
            throw new ADTException(String.format("%d is not present in the heap", position));
